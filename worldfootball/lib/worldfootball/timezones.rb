...
class CET   ## central european time helpers
...
class Timezone   ## nested inside UTC
   ...
   def local_time( year, month, day, hour, min, sec )
      @zone.local_time( year, month, day, hour, min, sec )
   end
...
def self.find_zone!( league:, season: )
   @zones ||= begin
                recs = read_csv( "#{Worldfootball.root}/config/timezones.csv" )
                zones = {}
                recs.each do |rec|
