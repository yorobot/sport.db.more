...
buf = String.new
buf << "= #{name}     # #{data['Results'].size} Teams\n\n"

 puts "= #{name}     # #{data['Results'].size} Teams"

## add stats
## buf << "# Teams  #{data['Results'].size}\n"
## buf << "\n"
...
data['Results'].each_with_index do |h,i|
    ...
    buf << "== #{team}     # #{players.size} Players\n\n"
    

    puts  "== [#{i+1}/#{results_count}]  #{team} - #{players.size} player(s)" 
...
##   only allow  alpha and space
 ...
 #      end
       ...
       buf << "\n"  if last_pos && last_pos != pos
        ...
        name_col = if firstIdCountry != idCountry
...
seasons.each do |season|
   data = read_json( "./misc/#{season}_squads.json" )

   jerseys = season >= 1954 ? true : false

   buf = ppsquads( data, name: "World Cup #{season}",
                         jerseys: jerseys )


   ## puts buf
   write_text( "#{outdir}/more/#{season}_squads.txt", buf )
end
...
__END__
     ...
     "PlayerName": [{"Locale": "en-GB", "Description": "Andrew AULD"}],
       "ShortName": [{"Locale": "en-GB", "Description": "Andrew AULD"}],
       "TeamType": null,
       "JerseyNum": null,
       "MatchesPlayed": null,
       "Position": 3,
       "BirthDate": "1900-01-26T00:00:00Z",
